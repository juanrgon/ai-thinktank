...
export class RosterSelectorComponent extends Container implements Focusable {
	...
	constructor(config: RosterSelectorConfig, callbacks: RosterSelectorCallbacks) {
		...
		this.addChild(new Text(theme.fg("accent", theme.bold("Thinktank Roster")), 0, 0));
		this.addChild(new Text(theme.fg("muted", "Saved roster. Changes apply immediately when you select."), 0, 0));
		this.addChild(new Spacer(1));
	...
	private updateSlots(): void {
		...
		for (let i = 0; i < THINKTANK_LAB_DEFINITIONS.length; i++) {
			...
			const labText = isActive ? theme.fg("accent", lab.shortName.padEnd(9)) : lab.shortName.padEnd(9);
			const modelText = entry
				? `${entry.model.provider}/${entry.model.id}:${entry.thinkingLevel}`
				: theme.fg("warning", `missing ${lab.providerCandidates.join("|")}`);
	...
	private updateList(): void {
		...
		if (this.filteredModels.length === 0) {
			this.listContainer.addChild(
				new Text(theme.fg("muted", `  No configured ${this.activeLab.shortName} models`), 0, 0),
			);
			this.listContainer.addChild(
				new Text(theme.fg("muted", `  Providers: ${this.activeLab.providerCandidates.join(", ")}`), 0, 0),
			);
			return;
		}
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const effort = isCurrent ? theme.fg("muted", `:${currentEntry?.thinkingLevel}`) : "";
			const checkmark = isCurrent ? theme.fg("success", " ✓") : "";
			this.listContainer.addChild(new Text(`${prefix}${modelText}${providerBadge}${effort}${checkmark}`, 0, 0));
		}

		if (startIndex > 0 || endIndex < this.filteredModels.length) {
			this.listContainer.addChild(
				new Text(theme.fg("muted", `  (${this.selectedModelIndex + 1}/${this.filteredModels.length})`), 0, 0),
			);
		}
		...
		if (selected) {
			...
			this.listContainer.addChild(
				new Text(
					theme.fg("muted", `  Effort: ${getThinktankSupportedThinkingLevels(selected.model).join(", ")}`),
					0,
					0,
				),
			);
